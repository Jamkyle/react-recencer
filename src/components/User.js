...
class User extends Component {
  ...
  componentWillReceiveProps(){
    const {modUser} = this.props
    localStorage.setItem('userMod', JSON.stringify(modUser))
  }

  render() {
    const { history, Sections, fields: { firstName, lastName, email, sections, admin },currentUser,modUser, handleSubmit} = this.props
    const { message } = this.state
    let adminLabel = null, userMod = localUser, form
    ...
    : _.map(Sections)
      ...
      if(currentUser.id === this.props.params.userId || currentUser.admin)
      {
        form = (
          <div>
      ...
      return (
        ...
        <button onClick={()=>{if(history.hasBack())history.goBack()}}> back </button>
        </div>
      )
